...
extension UIView {
    func embed(subview: UIView, padding: UIEdgeInsets = .zero, ignore: [UIRectEdge] = []) {
        ...
        if ignore.contains(.top) == false {
            let constraint = subview.topAnchor.constraint(equalTo: topAnchor, constant: padding.top)
        ...
        if ignore.contains(.left) == false {
            let constraint = subview.leftAnchor.constraint(equalTo: leftAnchor, constant: padding.left)
